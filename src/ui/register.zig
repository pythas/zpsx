...
pub const RegisterWindow = struct {
    ...
    pub fn draw(_: *Self, config: *WindowConfig, emulator: *Emulator) void {
        ...
        if (ig.igBegin("Registers", &config.visible, 0)) {
            ...
            ig.igText("SR:    0x%08X", cpu.cop0.data_registers[12]);
            ig.igText("Cause: 0x%08X", cpu.cop0.data_registers[13]);
            ig.igText("EPC:   0x%08X", cpu.cop0.data_registers[14]);

            ig.igSeparator();
            ig.igText("All CP0 Registers (sel 0)");
            for (0..32) |i| {
                ig.igText("r%d: 0x%08X", @as(i32, @intCast(i)), cpu.cop0.data_registers[i]);
                if (i % 2 == 0) {
                    ig.igSameLine();
                }
            }
